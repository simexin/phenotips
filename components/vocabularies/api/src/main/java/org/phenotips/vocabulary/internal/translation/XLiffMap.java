...
/**
 * A container for XLiffFiles that maps from vocabulary/language pairs to the corresponding XLiffFile or throws an
 * exception if none is to be found.
 *
 * @version $Id$
 */
class XLiffMap
{
    ...
    {
        this.map.put(getKey(vocabulary, lang), xliff);
    }

    /**
     * Get the xliff file for the vocabulary and language given or throw an exception if there isn't one.
     *
     * @param vocabulary the vocabulary
     * @param lang the language
    ...
    {
        XLiffFile xliff = this.map.get(getKey(vocabulary, lang));
        if (xliff == null) {
    ...
    {
        return this.map.remove(getKey(vocabulary, lang));
    }
}
