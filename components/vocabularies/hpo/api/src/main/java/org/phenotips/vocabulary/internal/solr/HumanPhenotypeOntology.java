...
public class HumanPhenotypeOntology extends AbstractOBOSolrVocabulary
{
    ...
    {
        // We need to make sure everything degrades gracefully, so if no locale or language is set to english anyway,
        // just use the old fields
        String lang = null;
        Locale currentLocale = this.localizationContext.getCurrentLocale();
        if (currentLocale != null) {
            lang = this.localizationContext.getCurrentLocale().getLanguage();
            if ("en".equals(lang)) {
                lang = null;
            } else if (lang != null) {
                lang = "_" + lang;
